...
tf.app.flags.DEFINE_integer('max-steps', 4500,
...
def main(_):
    ...
    with tf.variable_scope('model'):
        ...
        def logLoss(logitIn,classTen):
            ...
            val3 = tf.cond(tf.is_finite(val3),lambda: val3,lambda: tf.add(val6,tf.constant(0.00001))) 
            return tf.subtract(val3,val6)

        logProp = tf.map_fn(lambda (v1,v2):logLoss(v1,v2),(logits,y_),dtype=tf.float32)
        our_loss = tf.reduce_mean(logProp)
    ...
    # Used to display learned filters
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        #Adjust learning rate variables
        stdCheck = 0.017
        learningRate = 0.01
        ...
        test_accuracy = 0
        batch_count = 0
        # Used to get per Set of sgin accuracy
        ...
        print('test set: accuracy on test set: %0.4f' % test_accuracy)
    ...
    for x in predClass:
        if x == trueClass[i]:
            if x < 6 or x == 7  or x == 8:
                ...
                print("Class lable has impossible value x: {}".format(x))
        i += 1
